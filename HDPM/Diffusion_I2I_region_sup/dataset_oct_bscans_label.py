...
class AugmentedDataset_test(Dataset):
    def __init__(self, dataroot, cube_names=None, ret_info=False, skip_stride = 1):
        ...
        for cube_name in cube_names:
            ...
            for img_name in img_names:
                ...
                self.bscan_names.append(img_name)
                self.cube_names.append(cube_name)
        self.bscan_paths = self.bscan_paths[::skip_stride]
    ...
    def __getitem__(self, index):
        ...
        img = transforms.ToTensor()(img)
        if self.ret_info:
            return img, self.cube_names[index], self.bscan_names[index]
        else :
            return img
